...
import torch
import source.face_emotion_utils.test as face_test_model
...
if __name__ == "__main__":
    ...
    if ch == 1:
        ...
        if ch == 1:
           ...
           history=face_model.train_using_best_values()
           print('type=',type(history))
           face_model.plot_history(history,metric='loss')
           face_model.plot_history(history,metric='acc')
            ...
            model_path = config.FACE_MODEL_SAVE_PATH
            best_hyperparameters_path = config.FACE_BEST_HP_JSON_SAVE_PATH
            best_hyperparameters = utils.load_dict_from_json(best_hyperparameters_path)
            normalise = bool(best_hyperparameters['normalise'])

    # Load the model
            model = torch.load(model_path)
            model.to(config.device).eval()

    # Test the model accuracy in batches
            face_test_model.test_model_accuracy(model, normalise, best_hyperparameters_path, batch_size=64)
