...
def predict(input_file_name, model_save_path=config.AUDIO_MODEL_SAVE_PATH):
    ...
    with torch.no_grad():
        ...
        predicted_index = np.argmax(prediction_numpy)
        print(f"Predicted index: {predicted_index}")

        # Get the predicted emotion label
        emotion = config.EMOTION_INDEX[predicted_index]
        predicted_probability = prediction_numpy[predicted_index]
        
        print(f"Predicted emotion: {emotion} {round(predicted_probability, 2)}")

        ret_string = utils.get_softmax_probs_string(prediction_numpy, list(config.EMOTION_INDEX.values()))
        print(f"\n\n\nPrediction labels:\n{ret_string}")

    return emotion, round(predicted_probability, 2), ret_string
