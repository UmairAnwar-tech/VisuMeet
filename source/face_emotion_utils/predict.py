...
emotion_counts = {"Sad/Fear": 0, "Neutral": 0, "Happy": 0, "Angry": 0, "Surprise/Disgust": 0}
emotion_confidences = {"Sad/Fear": 0.0, "Neutral": 0.0, "Happy": 0.0, "Angry": 0.0, "Surprise/Disgust": 0.0}
total_count = 0
emotion_weights = {
    'Happy': 1.0,
    'Neutral': 0.8,
    'Surprise/Disgust': 0.7,
    'Sad/Fear': 0.3,
    'Angry': 0.4
}
total_count = 0
def update_emotion_confidence(emotion, confidence):
    global total_count
...
def calculate_average_confidences():
    average_confidences = {}
    for emotion, total_confidence in emotion_confidences.items():
        if emotion_counts[emotion] > 0:
            average_confidences[emotion] = total_confidence / emotion_counts[emotion]
        else:
            average_confidences[emotion] = 0.0
    return average_confidences

def calculate_overall_confidence():
    average_confidences = calculate_average_confidences()
    ...
    for emotion, avg_confidence in average_confidences.items():
        total_weighted_confidence += avg_confidence * emotion_weights[emotion]
    
    # Normalize the total weighted confidence to ensure it is between 0 and 1
    max_possible_confidence = sum(emotion_weights.values())
    overall_confidence = total_weighted_confidence / max_possible_confidence
...
def _get_prediction(
    ...
    if verbose:
        ...
        emotion = emotion_index_dict[prediction_index]
        confidence = max(pred_numpy)
        update_emotion_confidence(emotion, confidence)
        string_img = emotion + ": " + str(round(confidence * 100)) + "%"
        update_emotion_confidence(emotion, confidence)
    ...
    # Update global emotion confidence
    overall_confidence = calculate_overall_confidence()
    print("Overall Confidence Level:", overall_confidence) 
    # return_objs = (emotion_index_dict[prediction_index], prediction_index, list(pred_numpy), overall_confidence)
    #return_objs = (emotion_index_dict[prediction_index], prediction_index,  overall_confidence)
    return_objs = (emotion_index_dict[prediction_index], prediction_index, max(pred_numpy),  overall_confidence)
    if grad_cam:
