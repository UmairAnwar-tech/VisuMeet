...
def load_preprocessed_test_data(
        normalise,
        input_path=config.PREPROCESSED_TEST_IMAGES_FOLDER_PATH,
        save_name_suffix="_default",
):
    # Load from file
    print(f"\nLoading preprocessed files from: {input_path}")
...
def preprocess_timages(
        original_images_folders=config.ALL_EXTRACTED_tFACES_FOLDERS,
        output_path=config.PREPROCESSED_TEST_IMAGES_FOLDER_PATH,
        print_flag=True,
):
    all_face_land_dists_depths_X = []  # List of all the face landmarks distances and depths
    all_face_images_X = []  # List of all images
